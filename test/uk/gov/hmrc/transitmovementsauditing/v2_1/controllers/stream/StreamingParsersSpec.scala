...
package uk.gov.hmrc.transitmovementsauditing.v2_1.controllers.stream
...
class StreamingParsersSpec extends AnyFreeSpec with Matchers with ScalaFutures with TestActorSystem {
      ...
      (1 to 5).foreach {
        value =>
          s"~$value kb string is created" in {
            ...
            val sut        = new Harness(controllerComponents)
