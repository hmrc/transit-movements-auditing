...
class DetailsSpec extends AnyFreeSpec with Matchers with ScalaCheckDrivenPropertyChecks with ModelGenerators {
  ...
  "when Details is serialized, return an appropriate JsObject" in forAll(
    ...
    (metadata, payload) =>
      val actual = Details.detailsFormat.writes(Details(Some("SubmitDeclarationFailedEvent"), metadata, Some(payload)))
      val expected = Json.obj(
        "subType"  -> "SubmitDeclarationFailedEvent",
        "metadata" -> metadata,
        "payload"  -> payload
      )
  ...
  "when an appropriate JsObject is deserialized, return a Details" in forAll(
    ...
    (metadata, payload) =>
      ...
      val expected = Details(Some("SubmitDeclarationFailedEvent"), metadata, Some(payload))
